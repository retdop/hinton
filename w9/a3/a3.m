...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % % ret.input_to_hid = model.input_to_hid * wd_coefficient - (data.targets - class_input) * transpose(class_input .* (1 - class_input)) * model.hid_to_class * (hid_input .* (1 - hid_input)) * model.input_to_hid * hid_output;
  % ret.hid_to_class = model.hid_to_class * wd_coefficient - (data.targets - class_input) .* class_input .* (1 - class_input) * transpose(hid_output) ;
